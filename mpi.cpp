...
#include <vector>
#include <algorithm>
#include <mpi.h> // Required for distributed memory
...
int main(int argc, char **argv)
{
    ...
    // start timer after initialization and before file loading
    ...
    // broadcast dimensions to all workers so they can allocate memory for the image
    ...
    // broadcast full unblurred image to all workers
    MPI_Bcast(img_in, width * height * 3, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);

    // calculate workload for this specific worker
    int rows_per_proc = height / size;
    int y_start = rank * rows_per_proc;
    int y_end = (rank == size - 1) ? height : y_start + rows_per_proc; // last worker takes any remaining rows
    ...
    // compute blur on the local chunk
    blur(img_in, local_out, width, height, y_start, y_end);

    // set up variables for rank 0 to gather all chunks
    ...
    // gather results from all workers into img_out on rank 0
    ...
    if (rank == 0)
    {
        ...
        std::cout << "MPI time: " << time << " seconds" << std::endl;
